...
import type { CurrentRaffle, RaffleInfo } from '../web3/useAgentApi';
...
export function FeaturedRafflesCarousel({ onJoinRaffle, onCreateRaffle, onNavigate, onTriggerWinner, isDrawingMode = false, currentRaffle, activeRaffles = [] }: FeaturedRafflesCarouselProps) {
  ...
  useEffect(() => {
    const updateCountdown = () => {
      ...
      if (closesAt) {
        ...
        // Fallback: time until next hour
        const now = new Date();
        const nextHour = new Date(now);
        nextHour.setHours(now.getHours() + 1, 0, 0, 0);
        diff = nextHour.getTime() - now.getTime();
      }
  ...
  // Build slides from live data + static "create" slide
  const raffleSlides = (currentRaffle ? [currentRaffle, ...activeRaffles.filter(r => r.address !== currentRaffle.address)] : activeRaffles).map((raffle) => {
    const pool = parseFloat(raffle.totalPool);
    const typeEmoji = raffle.type === 'house' ? '🏠' : '🌐';
    return {
  ...
  const slides = [
    ...(raffleSlides.length > 0 ? raffleSlides : [{
      type: 'raffle' as const,
      title: 'No Active Raffles',
      prizeAmount: 'Check back soon!',
      totalPool: '',
      status: '',
      background: treasureChestImage,
      address: undefined as string | undefined,
      raffleType: null as string | null,
      closesAt: undefined as string | undefined,
    }]),
    {
      type: 'create' as const,
      title: 'Create Your Own Raffle',
      subtitle: 'Deposit $5 to start your own community raffle now!',
              ...
              <div className="relative z-10 p-6 h-full flex flex-col justify-between text-white">
                ...
                {!isFinalCountdown && !isInDrawingMode && (
                  <div className="flex justify-center">
                    <Button
                      onClick={() => onJoinRaffle((slides[currentSlide] as any)?.address)}
